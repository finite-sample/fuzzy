...
```
├── main.tex                 # Paper
├── si.tex                   # Supplementary: database schemas
├── references.bib           # 26 references
├── replicate.py             # Generates all tables from scratch
├── tables/                  # LaTeX tables (generated by replicate.py)
...
python replicate.py          # writes tables/*.tex
pdflatex main && bibtex main && pdflatex main && pdflatex main
pdflatex si && pdflatex si
```

`replicate.py` runs two simulations (toy: 10 villages, 51 firms; medium: 40 villages, 600 firms), computes all estimators (oracle, canonical, expanded, collapsed, MI), and writes six `.tex` tables that `main.tex` inputs. Runtime is ~2 minutes.
